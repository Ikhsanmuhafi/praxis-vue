<template>
  <v-container>
    <v-card>
        ...
        <v-list v-for="user in filterUsers" :key="user.name">
          <v-list-item-content>
            ...
            <v-list-item-subtitle
              >Alamat : {{ user.address }}</v-list-item-subtitle
            >
            <v-list-item-subtitle
              >Program : {{ user.program }}</v-list-item-subtitle
            >
            <v-list-item-subtitle
              >Status : {{ user.status }}</v-list-item-subtitle
            >
          </v-list-item-content>
          <v-btn
            class="ma-2"
          ...
          <v-btn
            ...
            >Tolak</v-btn
          >
          <v-btn class="ma-2" color="error" @click="delusr(user.name)"
            >hapus</v-btn
          >
        </v-list>
      ...
      <v-spacer></v-spacer>
      <v-btn class="ma-2" color="error" dark @click="handleLogout">
    ...
    filterUsers() {
      return this.$store.state.users.filter((user) => {
        if (user.isApproved === false) {
          user.status = "Tolak";
          this.$toast.error("Sukses DiTolak", {
            type: "error",
            position: "top-right",
            duration: 3000,
            dismissible: true,
          ...
          user.status = "Terima";
          this.$toast.success("Sukses DiTerima", {
            type: "success",
            position: "top-right",
            duration: 3000,
            dismissible: true,
